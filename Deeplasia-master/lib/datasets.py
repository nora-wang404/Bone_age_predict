...
class HandDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        bone_age = torch.Tensor([self.anno_df.boneage.iloc[index]])
    ...
    def __init__(
        ...
        mask_dirs=None,#["../data/masks/tensormask"],
        age_norm=(0, 1),
        ...
        print(self.mean_age, self.std_age)
        self._remove_if_mask_missing()

        self.anno_df["male"] = np.where(self.anno_df["male"] == "True", 1, 0).astype(float)
        self.anno_df["boneage"] = (
            self.anno_df["boneage"] - self.mean_age
        ) / self.std_age
    ...
    def _open_image(self, index: int, method=cv2.IMREAD_GRAYSCALE) -> (np.ndarray, str):
        img_path = os.path.join(
            ...
            str(self.anno_df["id"].iloc[index])+".png"
    ...
    def _remove_if_mask_missing(self):
        ...
        for d in self.mask_dir:
            masks_in_dir = np.array(
                [
                    os.path.exists(os.path.join(d, row["dir"], row["id"]))
...
class HandDatamodule(pl.LightningDataModule):
    def __init__(
        ...
        mask_dirs: List[str] = None,#["../data/masks/tensormask", "../data/masks/unet"],
    ...
    def _handle_data_splits(
        ...
        df_all = pd.read_csv(annotation_path)
        df_train = pd.read_csv(annotation_path.replace("boneage-training-dataset.csv","train_data.csv"))
        df_val = pd.read_csv(annotation_path.replace("boneage-training-dataset.csv","val_data.csv"))
        df_test = pd.read_csv(annotation_path.replace("boneage-training-dataset.csv","boneage-test-dataset.csv"))
       # split = pd.read_csv(split_path)
        ...
        #train = df_all.loc[df[split_column] == "train"]
        self.age_mean = df_all["boneage"].mean() #  均值和方差
        self.age_std = df_all["boneage"].std()
        train = df_train
        val = df_val
        test = df_val # df_test
    ...
    def get_inference_augmentation(width=512, height=512, rotation_angle=0, flip=False):
        return A.Compose(
            [
                ...
                A.RandomResizedCrop(height=height, width=width, scale=(1.0, 1.0), ratio=(1.0, 1.0)),
                ToTensorV2(),
            ],
            p=1,
        )
...
class InferenceDataset(HandDataset):
    ...
    def __init__(
       ...
       # self.mask_dir = mask_dirs
        ...
        path =  self.anno_df["id"]
        drop = [os.path.exists(os.path.join(self.img_dir,str(p)+".png")) for p in path]
    ...
    def _open_image(self, index: int, method=cv2.IMREAD_GRAYSCALE) -> (np.ndarray, str):
        img_path = os.path.join(
            self.img_dir,str( self.anno_df["id"].iloc[index])+".png"
        )
